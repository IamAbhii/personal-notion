...
import { appliedOps, blocks, pages, properties, propertyValues, views } from '../db/schema';
...
export async function resetWorkspace(db: Db, ctx: Ctx): Promise<number> {
  ...
  const statements: Statement[] = [
    ...
    // Views before pages because views reference database page ids.
    ...
    db.delete(views).where(eq(views.workspaceId, ctx.workspaceId)),
